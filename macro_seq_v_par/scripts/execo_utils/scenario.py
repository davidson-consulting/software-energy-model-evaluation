...
class Scenario :
    ...
    def setup(self, loadBalancer = None):
        ...
        self._client.uploadAnonConfiguration (self._hostSensorConfig)
    ...
    def _installPhoronix (self, vm) :
        ...
        self._client.launchAndWaitCmd ([vm], "mkdir -p /home/alice/.phoronix-test-suite", user=self._vmUsers[vm]);
        ...
        self._client.launchAndWaitCmd ([vm], "sudo cp /home/alice/.phoronix-test-suite/user-config.xml /root/.phoronix-test-suite/", user=self._vmUsers[vm]);
        self._client.launchAndWaitCmd ([vm], "sudo cp /home/alice/.phoronix-test-suite/user-config.xml /etc/phoronix-test-suite.xml", user=self._vmUsers[vm]);
    ...
    def _finalizePhoronixTest (self, vm):
        ...
        self._client.launchAndWaitCmd ([vm], "sudo cp user-config-no-conn.xml /home/alice/.phoronix-test-suite/user-config.xml", user=self._vmUsers[vm])
    ...
    # ***********************************************
    def store (self):
        logs = self._client.downloadAnonResults ()
        logs = logs | self._client.downloadAnonResultsVM ()

        self._client.killAnon ()
        self._client.killAllVMs ()
        self._client.killDioMonitor () 
