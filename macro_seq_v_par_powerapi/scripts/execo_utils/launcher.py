...
class ExecoClient :
    ...
    def configureNodes (self, withInstall = True, withDownload = True) :
        if (withInstall) : 
            ...
            self.uploadFiles (self._hnodes, ["../../libs/powerapi/config-formula.json", "../../libs/powerapi/config-sensor.json", "../../libs/powerapi/docker-compose.yaml"], "./test")
    ...
    # ***************************
    def startPowerapi (self):
        self.launchAndWaitCmd (self._hnodes, "sudo rm -rf /tmp/sensor_output ; mkdir -p /tmp/sensor_output ; sudo chmod 777 /tmp/sensor_output")
        self.launchCmd (self._hnodes, "cd test ; docker compose up -V -d mongo")
        time.sleep (5)
        self.launchCmd (self._hnodes, "cd test ; docker compose up -V -d power-api-sensor")
        time.sleep (10)
    ...
    # ***************************
    def killPowerapi (self):
        self.launchCmd (self._hnodes, "cd test ; docker compose stop power-api-sensor")
        time.sleep (1)
        self.launchAndWaitCmd (self._hnodes, "cd test ; docker compose up -V power-api-formula")
        self.launchCmd (self._hnodes, "cd test ; docker compose down -v")
        for v in self._vmInfos :
            print ("CGROUP : ", v)
            self.launchAndWaitCmd (self._vmInfos [v], "sudo cgdelete -g perf_event:{0}".format (v))
    ...
    # ***************************
    def downloadPowerapiResults (self, outputDir = "/tmp/"):
        def _downloadPowerapiResults (self, node, outputDir) :
            final = {}
            path = outputDir + str (node.address)
            if not (os.path.exists (path) and os.path.isdir (path)):
                os.mkdir (path)

            self.downloadFiles ([node], ["/tmp/sensor_output/*"], path)
        ...
        for node in self._hnodes :
            logger.info (f"Download powerapi result for {node}")
            final = final | _downloadPowerapiResults (self, node, outputDir)
    ...
    def configureVMCgroups (self):
        for v in self._vmInfos :
            print ("CGROUP : ", v)
            self.launchAndWaitCmd (self._vmInfos [v], "sudo cgcreate -g perf_event:{0}".format (v))
            self.launchAndWaitCmd (self._vmInfos [v], "sudo cgclassify -g perf_event:{0} $(ps aux | grep {0}".format (v) + " | head -n 1 | awk '{print $2}')")
